      ...
      <div class="form-container">
        ...
        <div class="contact-form">
          ...
          <div class="form-field">
            <label class="field-label">参加形式</label>
            <div class="field-input">
              <div class="confirm-value">{{ getAttendanceTypeText(formData.attendanceType) }}</div>
            </div>
          </div>
          ...
          <!-- Special Requests -->
          <div class="form-field">
            <label class="field-label">特別なご要望・備考</label>
            <div class="field-input">
              <div class="confirm-value content-value">{{ formData.specialRequests || 'なし' }}</div>
            </div>
          </div>
  ...
  mounted() {
    ...
    if (params.get('formData')) {
      try {
        ...
        this.$router.push('/seminar/application');
      }
    } else {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        // セミナー申し込み用のペイロード
        const payload = {
          seminar_id: this.formData.seminarId,
          name: `${this.formData.lastName} ${this.formData.firstName}`.trim(),
          email: this.formData.email,
          phone: this.formData.phone || '',
          company: this.formData.companyName || '',
          position: this.formData.position || '',
          attendance_type: this.formData.attendanceType,
          special_requests: this.formData.specialRequests || '',
          privacy_agreement: this.formData.privacyAgreement
        }

        const res = await apiClient.registerForSeminar(this.formData.seminarId, payload)
        ...
        const applicationNumber = res?.data?.application_number
          || (res?.data?.registration_id ? `SEM-${String(res.data.registration_id).padStart(6, '0')}` : null)
          || (res?.registration_id ? `SEM-${String(res.registration_id).padStart(6, '0')}` : null)
          || `SEM-${Date.now()}`

        this.$router.push(`/seminar/application/complete?applicationNumber=${encodeURIComponent(applicationNumber)}`)
      } catch (err) {
