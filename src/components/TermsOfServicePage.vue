...
export default {
  ...
  mounted() {
    ...
    try {
      ...
      // Re-render when images map changes to avoid resize dependency
      try {
        const readImages = () => {
    ...
    // Reflect admin edits instantly: listen for page content cache and media updates
    try {
      this.__lastReloadAt = 0
      this.__reloading = false
      this.__onStorage = (ev) => {
        const k = ev && ev.key ? String(ev.key) : ''
        if (k === 'page_content_cache:terms') {
          const now = Date.now()
