      ...
      <div class="content-container" v-if="!isEditPreview || !hasHtml">
        ...
        <div class="policy-section">
          ...
          <div class="list-content">
            <CmsText pageKey="privacy" fieldKey="purpose_list" tag="div" type="html" :fallback="'(1) サービスの提供、キャンペーンなどを行うため<br>(2) サービスに関する情報またはキャンペーン情報を提供するため<br>(3) お客様から寄せられたご意見、ご要望にお答えするため<br>(4) サービスの開発・改善を目的とした調査・検討を行うため<br>(5) サービスに関する統計的資料を作成するため'" />
          </div>
        </div>
        ...
        <div class="policy-section">
          ...
          <div class="list-content">
            <CmsText pageKey="privacy" fieldKey="disclosure_list" tag="div" type="html" :fallback="'(1) 法令により情報の開示が求められる場合<br>(2) 人の生命、身体または財産の保護のために必要があると当研究所が判断した場合<br>(3) 国の機関もしくは地方公共団体またはその委託を受けた者が法令の定める事務を遂行することに対して協力することその他公共の利益のために特に必要があると当研究所が判断した場合<br>(4) お客様または当研究所の権利の確保のために必要であると当研究所が判断した場合<br>(5) 業務遂行に必要な限度で個人情報の取扱いを委託する場合'" />
          </div>
        </div>
        ...
        <div class="policy-section">
          ...
          <p class="section-content">
            <CmsText pageKey="privacy" fieldKey="correction_body" tag="div" type="html" :fallback="'１.お客様は、当研究所所定の手続きにより、以下の請求を行うことができます。<br>　(1) 当研究所の保有する自己の個人情報が誤った情報でないことを確認すること<br>　(2) 当研究所の保有する自己の個人情報が誤った情報である場合に、それを訂正または削除すること<br>２.当研究所は、前項(２)の個人情報の訂正または削除の可否を決定した場合には、遅滞なく、当該お客様に通知します。<br>３.これらの請求を希望される場合には、お問い合わせフォームよりご連絡ください。'" />
          </p>
        </div>
...
export default {
  ...
  mounted() {
    try {
      ...
      // Re-render when images map changes (avoid resize dependency)
      try {
        const readImages = () => {
    ...
    // Instant reflect: listen for storage/visibility/media updates
    try {
      this.__lastReloadAt = 0
      this.__reloading = false
      this.__onStorage = (ev) => {
        const k = ev && ev.key ? String(ev.key) : ''
        if (k === 'page_content_cache:privacy') {
          const now = Date.now()
      ...
      // media registry broadcast (e.g. KV update)
